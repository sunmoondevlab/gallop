...
TEST_F(TranslateArgsTest, TranslateArgs) {
  ...
  ASSERT_THAT(args.getLlvmOption().getDstRootDir(),
              EndsWith("/gallop/example/test_dst_root"));
  ASSERT_EQ(args.getArgs().size(), (size_t)0);
  ASSERT_EQ(File::isDirectory("../example/test_dst_root"), true);
  ASSERT_EQ(File::rmdir("../example/test_dst_root"), true);
};
