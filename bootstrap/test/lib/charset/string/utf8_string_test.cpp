...
TEST(Utf8StringTest, ToUtf32Data) {
  // BE BOM
  ASSERT_EQ(Utf8::toUtf32Data("\xef\xbb\xbf", false, true),
            std::vector<int32_t>{static_cast<int32_t>(0x0000feff)});
  // LE BOM
  ASSERT_EQ(Utf8::toUtf32Data("\xef\xbb\xbf", true, true),
            std::vector<int32_t>{static_cast<int32_t>(0xfffe0000)});
  // without BOM
  // BE BOM
  ASSERT_EQ(Utf8::toUtf32Data("\xef\xbb\xbf\xef\xbb\xbf", false, false),
            std::vector<int32_t>());
  // LE BOM
  ASSERT_EQ(Utf8::toUtf32Data("\xef\xbb\xbf\xef\xbb\xbf", true, false),
            std::vector<int32_t>());
  // BE
...
TEST(Utf8StringTest, ToUtf16Data) {
  // BE BOM
  ASSERT_EQ(Utf8::toUtf16Data("\xef\xbb\xbf", false, true),
            std::vector<int16_t>{static_cast<int16_t>(0xfeff)});
  // LE BOM
  ASSERT_EQ(Utf8::toUtf16Data("\xef\xbb\xbf", true, true),
            std::vector<int16_t>{static_cast<int16_t>(0xfffe)});
  // // without BOM
  // BE BOM
  ASSERT_EQ(Utf8::toUtf16Data("\xef\xbb\xbf\xef\xbb\xbf", false, false),
            std::vector<int16_t>());
  // LE BOM
  ASSERT_EQ(Utf8::toUtf16Data("\xef\xbb\xbf\xef\xbb\xbf", true, false),
            std::vector<int16_t>());
  // BE
...
TEST(Utf8StringTest, ToUtf8Data) {
  std::vector<char> expectUtf8;
  // BOM
  expectUtf8 = {static_cast<char>(0xef), static_cast<char>(0xbb),
                static_cast<char>(0xbf)};
  ASSERT_EQ(Utf8::toUtf8Data("\xef\xbb\xbf", true), expectUtf8);
  // without BOM
  expectUtf8 = {};
  ASSERT_EQ(Utf8::toUtf8Data("\xef\xbb\xbf\xef\xbb\xbf", false), expectUtf8);
  ...
  ASSERT_EQ(Utf8::toUtf8Data(" 0Aa~"
                             "\xc2\xc2\x80 ÿĀ߿\xe0ࠀ鿿ꀀퟻ豈\xf0𐃺𠀀鼻𰻝𱁬\xc2\xc0"
                             "\xe0\xbf\xc0\xf0\xbf\xbf\xc0あいうえお",
                             false),
            expectUtf8);
};
TEST(Utf8StringTest, ToUtf8String) {
  // BOM
  ASSERT_EQ(Utf8::toUtf8String("\xef\xbb\xbf"), "");
  ASSERT_EQ(Utf8::toUtf8String(std::vector<char>{
