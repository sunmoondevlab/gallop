...
const std::map<TokenTypeEnum, std::string> TokenType::_map = {
    ...
    {TokenTypeEnum::literalCharCode, "char code literal"},
    {TokenTypeEnum::literalString, "string literal"},
    {TokenTypeEnum::literalRawString, "string(raw) literal"},
    ...
    {TokenTypeEnum::symbolCharacterGreaterGreaterGreater, "operator(>>>)"},
    //  : other symbols
    ...
    {TokenTypeEnum::symbolCharacterBackquote, "raw string separator(`)"},
...
bool TokenType::isSymbolCharacterToken(const enum TokenTypeEnum &tokenType_) {
  return (uint32_t(tokenType_) >> 24) == TokenTypeCategoryEnum::symbolCharacter;
};
bool TokenType::isCommentOut(const enum TokenTypeEnum &tokenType_) {
  return (uint32_t(tokenType_) >> 24) == TokenTypeCategoryEnum::commentOut;
};
bool TokenType::isCharCodesDelimiter(const enum TokenTypeEnum &tokenType_) {
  return tokenType_ == TokenTypeEnum::symbolCharacterSinglequote ||
         tokenType_ == TokenTypeEnum::symbolCharacterDoublequote ||
         tokenType_ == TokenTypeEnum::symbolCharacterBackquote;
};
