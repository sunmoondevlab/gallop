...
bool File::touch(const std::string &filename_) {
  std::error_code ec;
  llvm::raw_fd_ostream of(filename_, ec);
  of.close();
  if (ec) {
    return false;
  }
...
std::string File::replacePathDirectory(const std::string &filename_,
  ...
  if (absDestDir.size() > 1) {
    absDestDir.push_back(std::filesystem::path::preferred_separator);
  }
  size_t len = absSrcDir.length();
  std::string destFilename = absFilename;
  std::filesystem::path destFilepath(
      destFilename.replace(pos, len, absDestDir));
  return destFilepath.string();
};
...
std::vector<char> File::readAllBinary(const std::string &filename_) {
  ...
  if (file == nullptr) {
    llvm::outs() << "file not found " + filename_ << "\n";
    throw "";
  }
  fpos_t fsize;
