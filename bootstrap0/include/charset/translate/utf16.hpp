...
class Utf16Decoder {
public:
  Utf16Decoder(const std::vector<char> &buffer_);
  ~Utf16Decoder() {};
...
class Utf16Encoder {
public:
  Utf16Encoder(const std::string &buffer_, const bool littleEndianFlag_,
               const bool utf16BomFlag_);
  ~Utf16Encoder() {};
  Utf16Encoder(const Utf16Encoder &rhs);
  Utf16Encoder &operator=(const Utf16Encoder &rhs);
  std::vector<int32_t> getUtf16RunesBuffer();
  std::vector<char> getUtf16Buffer();
  size_t getUtf16RunesBufferLen();
  size_t getUtf16BufferLen();
